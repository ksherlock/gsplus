...
#define BIGEND(a)                                                              \
  ((((a) >> 24) & 0xff) + (((a) >> 8) & 0xff00) + (((a)&0xff00) << 8) +        \
   (((a)&0xff) << 24))
...
STRUCT(Page_info) { Pg_info rd_wr; };
...
#define clear_halt() clr_halt_act()

#define GET_PAGE_INFO_RD(page) (page_info_rd_wr[page].rd_wr)
...
#define SET_PAGE_INFO_WR(page, val)                                            \
  ;                                                                            \
  page_info_rd_wr[0x10000 + PAGE_INFO_PAD_SIZE + (page)].rd_wr = (Pg_info)val;
...
#define disk_printf                                                            \
  if (DO_VERBOSE && (Verbose & VERBOSE_DISK))                                  \
  printf
#define irq_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_IRQ))                                   \
  printf
#define clk_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_CLK))                                   \
  printf
#define shadow_printf                                                          \
  if (DO_VERBOSE && (Verbose & VERBOSE_SHADOW))                                \
  printf
#define iwm_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_IWM))                                   \
  printf
#define doc_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_DOC))                                   \
  printf
#define adb_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_ADB))                                   \
  printf
#define scc_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_SCC))                                   \
  printf
#define test_printf                                                            \
  if (DO_VERBOSE && (Verbose & VERBOSE_TEST))                                  \
  printf
#define vid_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_VIDEO))                                 \
  printf
#define mac_printf                                                             \
  if (DO_VERBOSE && (Verbose & VERBOSE_MAC))                                   \
  printf
